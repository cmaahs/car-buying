...
#2016
#$modelYearCode = "IUD201605"
#2017
$modelYearCode = "IUD201705" 
...
$zipCodeList | Export-Clixml "C:\Temp\GrandCaravan\zipCodeList.clixml"
...
$allDealerList | Export-Clixml "C:\Temp\GrandCaravan\allDealerList.clixml"



#http://www.dodge.com/inventory/?modelYearCode=IUD201605&radius=25&ccode=CUD201605RTKH53H&llp=2DQ&ref=bmo&zip=55311&variation=SE%20PLUS&optionCodes=ERB,NHK,DG2,Z1A,SDC,N7,AHS,ADZ,AT4,WFH,TU3,RBZ,CYC,PAU,29Q&drive=FWD
foreach ( $dealerKey in $allDealerList.Keys ) 
{
    ...
    $uri = "http://www.dodge.com/hostd/inventory/getinventoryresults.json?attributes=drive:FWD&dealerCodes=$($dealerKey)&includeIncentives=N&matchType=X&modelYearCode=$($modelYearCode)&pageNumber=1&pageSize=10&radius=50&sortBy=0&zip=$($allDealerList[$dealerKey].Zip.Substring(0,5))"
    
    $result = Invoke-WebRequest -Uri $uri

    $inventoryData = $result.Content | ConvertFrom-Json

    $inventoryData | Export-Clixml -Path "C:\Temp\GrandCaravan\Inventory_$($dealerKey)_Page1.clixml"
    ...
    foreach ( $vehicle in $inventoryData.result.data.vehicles )
    {
        ...
        if ( -Not ( Test-Path -Path "C:\Temp\GrandCaravan\$($vehicle.vin).pdf" ) ) 
        {
            $carsDownloaded++
            $windowStickerURI = "http://www.dodge.com/hostd/windowsticker/getWindowStickerPdf.do?vin=$($vehicle.vin)"
            $stickerName = "C:\Temp\GrandCaravan\$($vehicle.vin).pdf"
            ...
            $optionsName = "C:\Temp\GrandCaravan\$($vehicle.vin)_Options.xml"
            ...
            $dealerName = "C:\Temp\GrandCaravan\$($vehicle.vin).htm"
            ...
            if ( $lowPrice -ne "0" ) 
            {
                Write-Output $lowPrice | Out-File -FilePath "C:\Temp\GrandCaravan\$($vehicle.vin).txt" -Force -Encoding ascii
            }
        }
    }
    ...
    {
        $uri = "http://www.dodge.com/hostd/inventory/getinventoryresults.json?attributes=drive:FWD&dealerCodes=$($dealerKey)&includeIncentives=N&matchType=X&modelYearCode=$($modelYearCode)&pageNumber=$($pageNum)&pageSize=10&radius=50&sortBy=0&zip=$($allDealerList[$dealerKey].Zip.Substring(0,5))"

        $result = Invoke-WebRequest -Uri $uri

        $inventoryData = $result.Content | ConvertFrom-Json
        $inventoryData | Export-Clixml -Path "C:\Temp\GrandCaravan\Inventory_$($dealerKey)_Page$($pageNum).clixml"
