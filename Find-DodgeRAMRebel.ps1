...
{
	$zipCodeList | Export-Clixml "C:\Temp\RAM\ZipCodeList.clixml"
} else {
	$zipCodeList = Import-Clixml "C:\Temp\RAM\ZipCodeList.clixml"
}
$dealersDownloaded = 0
$skippedDealers = 0
$allDealerList = @{}
...
foreach ( $zipCode in $zipCodeList ) 
{
    if ( $zipCode.ZipCode.Length -eq 5 ) 
    {
        ...
        $uri = "http://www.ramtrucks.com/hostd/getlocatedealers.json?zipCode=$($zipCode.ZipCode)&zipDistance=200&isRamTrucks=Y&modelYearCode=IUT201713"
        $result = Invoke-WebRequest -Uri $uri
...
{
	$allDealerList | Export-Clixml "C:\Temp\RAM\AllDealerList.clixml"
} else {
	$allDealerList = Import-Clixml "C:\Temp\RAM\AllDealerList.clixml"
}
...
foreach ( $dealerKey in $allDealerList.Keys ) 
{
   ...
   #Write-Verbose "Getting Data for $($zipCode.ZipCode)" -Verbose
   #$uri = "http://www.ramtrucks.com/hostd/getlocatedealers.json?zipCode=$($zipCode.ZipCode)&zipDistance=200&isRamTrucks=Y&modelYearCode=IUT201713"
   #$result = Invoke-WebRequest -Uri $uri
   #
   #$dealerData = $result.Content | ConvertFrom-Json
   #
   #$dealerData | Export-Clixml "C:\Temp\RAM\DealerData_$($zipCode.ZipCode).clixml"

    #Perhaps LOOP through all DEALER list and just pull inventory for each dealer rather than by radius?        
    #http://www.ramtrucks.com/hostd/inventory/getinventoryresults.json?dealerCodes=68129&includeIncentives=N&matchType=X&modelYearCode=IUT201713&pageNumber=1&pageSize=10&radius=50&sortBy=0&variation=REBEL&zip=55374
    #http://www.ramtrucks.com/hostd/inventory/getinventoryresults.json?dealerCodes=68129,68978&includeIncentives=N&matchType=X&modelYearCode=IUT201713&pageNumber=1&pageSize=10&radius=50&sortBy=0&variation=REBEL&zip=55374

    #$uri = "http://www.ramtrucks.com/hostd/inventory/getinventoryresults.json?includeIncentives=N&matchType=X&modelYearCode=IUT201713&pageNumber=1&pageSize=10&radius=200&sortBy=0&variation=REBEL&zip=$($zipCode.ZipCode)"
    $uri = "http://www.ramtrucks.com/hostd/inventory/getinventoryresults.json?dealerCodes=$($dealerKey)&includeIncentives=N&matchType=X&modelYearCode=IUT201713&pageNumber=1&pageSize=10&radius=50&sortBy=0&variation=REBEL&zip=$($allDealerList[$dealerKey].Zip.Substring(0,5))"
    $result = Invoke-WebRequest -Uri $uri
	...
	{
    $inventoryData | Export-Clixml -Path "C:\Temp\RAM\Inventory_$($dealerKey)_Page1.clixml"

    foreach ( $vehicle in $inventoryData.result.data.vehicles )
    {
            
        if ( -Not ( Test-Path -Path "C:\Temp\RAM\$($vehicle.vin).pdf" ) ) 
        {
            $carsDownloaded++
            $windowStickerURI = "http://www.ramtrucks.com/hostd/windowsticker/getWindowStickerPdf.do?vin=$($vehicle.vin)"
            $stickerName = "C:\Temp\RAM\$($vehicle.vin).pdf"
            Invoke-WebRequest -Uri $windowStickerURI -OutFile $stickerName
    
            #$stockImageURI = "http://www.ramtrucks.com/mediaserver/iris?$($vehicle.mvDynamicImageURL)pov=fronthero&width=960&height=325&bkgnd=&resp=jpg&cut="
            #Write-Output $stockImageURI
            #$imageName = "C:\Temp\RAM\$($vehicle.vin).jpg"
            #Invoke-WebRequest -Uri $stockImageURI -OutFile $imageName
    
            $OptionsXmlUri = "http://www.ramtrucks.com/monroney/MonroneyXMLServlet?Vin=$($vehicle.vin)"
            $optionsName = "C:\Temp\RAM\$($vehicle.vin)_Options.xml"
            ...
            $dealerName = "C:\Temp\RAM\$($vehicle.vin).htm"
            ...
            if ( $lowPrice -ne "0" ) 
            {
                Write-Output $lowPrice | Out-File -FilePath "C:\Temp\RAM\$($vehicle.vin).txt" -Force -Encoding ascii
            }
        }
    }
    ...
    {
        #$uri = "http://www.ramtrucks.com/hostd/inventory/getinventoryresults.json?includeIncentives=N&matchType=X&modelYearCode=IUT201713&pageNumber=$($pageNum)&pageSize=10&radius=200&sortBy=0&variation=REBEL&zip=$($zipCode.ZipCode)"
        $uri = "http://www.ramtrucks.com/hostd/inventory/getinventoryresults.json?dealerCodes=$($dealerKey)&includeIncentives=N&matchType=X&modelYearCode=IUT201713&pageNumber=$($pageNum)&pageSize=10&radius=50&sortBy=0&variation=REBEL&zip=$($allDealerList[$dealerKey].Zip.Substring(0,5))"
        $result = Invoke-WebRequest -Uri $uri

        $inventoryData = $result.Content | ConvertFrom-Json
        $inventoryData | Export-Clixml -Path "C:\Temp\RAM\Inventory_$($dealerKey)_Page$($pageNum).clixml"
